...
serve(async (req) => {
  try {
    ...
    console.log(`🌍 Heure UTC (BDD): ${utcTimeForDB} | 🗼 Heure Paris: ${parisTime}`);
    ...
    */
    ...
    if (parisTime.startsWith(TASK_REMINDER_TIME)) {
        try {
            ...
            console.log(`🔍 Recherche des tâches avec deadline demain (${tomorrowStr})...`)
            ...
            if (tasksError) {
                ...
                for (const t of tasks) {
                    ...
                    if (!notificationsByUser[t.user_id]) notificationsByUser[t.user_id] = []
      ...
      console.log(`ℹ️ Pas de rappel de tâches à cette heure (${parisTime}), tâche envoyée seulement à ${TASK_REMINDER_TIME}`)
    }
    ...
    for (const sub of subscriptions) {
      ...
      if (!userRoutines || userRoutines.length === 0) {
        ...
        notificationPromises.push(Promise.resolve({ success: false, error: `No routines/tasks for user ${sub.user_id}`, userId: sub.user_id }))
        continue
      }
      ...
      if (routinesOnly.length > 0) {
        ...
        // messages randomisés pour une routine unique (conserver le comportement précédent)
        const routineMessages = [
          `C'est l'heure ! Ta routine \"{title}\" n'attend que toi. 🚀`,
          `Toc toc ! On oublie pas \"{title}\" ? On lâche rien ! 💪`,
          `Petit rappel amical : c'est le moment de briller avec : {title}. ✨`,
          `Hop hop hop ! \"{title}\" ne va pas se faire toute seule. 😉`
        ]

        const body = routinesOnly.length === 1
          ? routineMessages[Math.floor(Math.random() * routineMessages.length)].replace('{title}', routinesOnly[0].title)
          : `C'est l'heure de tes routines : ${routinesOnly.map(r => r.title).join(', ')}`

        notificationPromises.push((async () => {
          try {
            const payload = JSON.stringify({ title: 'Mes Routines', body, tag })
            await webpush.sendNotification(sub.subscription, payload)
            console.log(`[PUSH SUCCESS] Routines User: ${sub.user_id.substring(0,8)}`)
            return { success: true, userId: sub.user_id, itemType: 'routines' }
          } catch (error) {
            console.error(`[PUSH ERROR] Routines User: ${sub.user_id.substring(0,8)} | Error: ${error.message}`)
            if (error.statusCode === 410) {
              console.warn(`[CLEANUP] Removing expired subscription for user: ${sub.user_id}`)
              await supabaseClient.from('push_subscriptions').delete().match({ user_id: sub.user_id, endpoint: sub.subscription.endpoint })
            }
            return { success: false, error: error.message, userId: sub.user_id, itemType: 'routines' }
          }
        })())
      }
      ...
      if (tasks.length > 0) {
        ...
        // messages dédiés pour les tâches
        const taskMessages = [
          `Rappel : ta tâche "{title}" a une deadline demain. Pense à t'en occuper ! ⏳`,
          `N'oublie pas "{title}" — deadline demain. Besoin d'un coup de boost ? 💪`,
          `Deadline demain pour "{title}" — tu peux le faire ! 🔔`
        ]

        const body = tasks.length === 1
          ? taskMessages[Math.floor(Math.random() * taskMessages.length)].replace('{title}', tasks[0].title)
          : `Rappel tâches (deadline demain) : ${tasks.map(t => t.title).join(', ')}`

        notificationPromises.push((async () => {
          try {
            const payload = JSON.stringify({ title: 'Rappel tâche', body, tag })
            await webpush.sendNotification(sub.subscription, payload)
            console.log(`[PUSH SUCCESS] Tasks User: ${sub.user_id.substring(0,8)}`)
            return { success: true, userId: sub.user_id, itemType: 'tasks' }
          } catch (error) {
            console.error(`[PUSH ERROR] Tasks User: ${sub.user_id.substring(0,8)} | Error: ${error.message}`)
            if (error.statusCode === 410) {
              console.warn(`[CLEANUP] Removing expired subscription for user: ${sub.user_id}`)
              await supabaseClient.from('push_subscriptions').delete().match({ user_id: sub.user_id, endpoint: sub.subscription.endpoint })
            }
            return { success: false, error: error.message, userId: sub.user_id, itemType: 'tasks' }
          }
        })())
      }
    }
